 ...
 * @date    2024
 * 
 * @brief   Software code used to validate the software and hardware developped on this 
            project.
 * 
...
*    @param title Test title to be used to report the purpose of the measure
*    @param cmd   String command to send to the SCPI engine
*    @param expected_result   String to validate with the result returned by the commansd
*    @param counter Structure who content the result of test. Be used by final report
*    @param buffer  Pointer to circular buffer, who contains the error
*    @return
*/
 ...
 */
 void internal_test_sequence(const char *testboard_num, uint8_t run){
  ...
  while(lp){
    ...
    if (run == 0) { 
      in[0] = 0;  // initialize value
      sprintf(sval,"\n\n\tInternal Test Sequences\n");  uart_puts(UART_ID, sval); // Send string 
      sprintf(sval,"1- Selftest using only selftest board, no check of onewire\n");  uart_puts(UART_ID, sval); // Send string 
      sprintf(sval,"2- Selftest run only if selftest board is installed, onewire validation\n");  uart_puts(UART_ID, sval); // Send string 
      sprintf(sval,"3- Selftest using selftest board and loopback connector\n");  uart_puts(UART_ID, sval); // Send string 
      sprintf(sval,"4- Selftest of instruments in manual mode using selftest board\n");  uart_puts(UART_ID, sval); // Send string 
      sprintf(sval,"5- Test of SCPI command,selftest board is required\n");  uart_puts(UART_ID, sval); // Send string 
      sprintf(sval,"0- Exit test sequence\n");  uart_puts(UART_ID, sval); // Send string 
    ...
    } else {  // if run > 0, execute test directly and exit
       tnb = run;   // assign testnumber tnb to value of parameter run
       lp = false;  // exit while loop when test completed
    }

   // execute selftest prgram
   switch (tnb){
    case 0 :
    ...
    if (rtn == 1) {
      sprintf(sval,"Selftest board not detected reading onewire\n");  uart_puts(UART_ID, sval); // Send string 
    }
...
uint8_t test_selftest(const char *testboard_num, uint8_t run) {
 ...
 char sval[80];   /**< array who will contains parts of SCPI command */ 
 ...
 uart_puts(UART_ID, sval); // Send result
...
if (run >= 2) {  
    ...
    result = onewire_read_info(&sdata, ADDR_INFO,NB_INFO,1);
    fprintf(stdout,"\tSelftest 1-wire: %s\n", sdata);  // send message to debug port
      // Use strstr to find the substring
    char *pos = strstr(sdata,testboard_num);
    if (pos != NULL) {
...
if (run == 3) {
 ...
 // send result string to serial port
  sprintf(sval,"SELFTEST RESULTS: \n NbTotal: %d, NbGood: %d, NbBad: %d, NbError: %d\n",ctest.total,ctest.good,ctest.bad,ctest.error);  // build string to return
  uart_puts(UART_ID, sval); // Send result
  ...
  sprintf(sval,"SELFTEST COMPLETED \n");  // build string to return
  uart_puts(UART_ID, sval); // Send string
  
}


/*! @brief - The function perform the seltest of the instruments connected to the 
...
// Setup I2C configuration to be used to communicate with selftest Board
 ...
 sprintf(sval,"Connect DMM to Sense pins (+:J20-5 & -:J20-6)\n");  uart_puts(UART_ID, sval); // Send string 
 sprintf(sval,"Set DMM to be able to read 10 ohms resistors\n");  uart_puts(UART_ID, sval); // Send string 
 sprintf(sval,"Test 25.0 Verify if ohmeter value is between 10 and 16 Ohm, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1);sleep_ms(500); // wait for input char
...
// Using DMM in resistance mode, measure test resistors R5 (4.7 ohm), R6 (10 ohm) and R7 (4.7 ohm)
 ...
 sprintf(sval,"Set DMM to be able to read 25 ohms resistors, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 sprintf(sval,"Test 25.1 Verify if DMM ohmeter value is between 20 and 23 Ohm, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1);sleep_ms(500); // wait for input char
 TEST_SCPI_INPUT("GPIO:OUT:DEV0:GP0 0 \r\n");  // Open K6
...
// Using DMM in resistance mode, measure selftest relays path when VM1 is grounded by K4
 ...
 sprintf(sval,"Test 25.2 Verify if DMM ohmeter value is between 0 and 5 Ohm, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1);sleep_ms(500); // wait for input char
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  0 \r\n");  // Open K4
 ...
 uart_read_blocking(UART_ID,in,1); sleep_ms(500); // wait for input char
 TEST_SCPI_INPUT("ROUT:OPEN:OC OC1 \r\n");  // Open K10
 sprintf(sval,"Test 25.3 Verify if DMM Voltmeter value is between 4.75V and 5.25V, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); // wait for input char
 ...
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 TEST_SCPI_INPUT("GPIO:OUT:DEV0:GP1 1 \r\n");  // Close K5
 sprintf(sval,"Test 25.4 Verify if DMM Ammmeter value is between 48mA and 52mA, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 ...
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP8 1 \r\n");  //Close K13
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  1 \r\n");  // Close K4
 sprintf(sval,"Test 25.5 Verify if DMM Ammmeter value is between 300mA and 400mA, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  0 \r\n");  // Open K4
 TEST_SCPI_INPUT("GPIO:OUT:DEV0:GP1 0 \r\n");  // Open K5
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP8 0 \r\n");  // Open K13
...
// Trigger signal will be activated and voltage will be measured with DMM
 ...
 sprintf(sval,"Set DMM to voltmeter mode (5V Range), press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 sprintf(sval,"Test 25.6 Verify if DMM value value is between 0V  and 0.1 Volt, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  1 \r\n");  // DVM_TRIG =1
 sprintf(sval,"Test 25.7 Verify if DMM value value is between 2V  and 3.3 Volt, press enter\n");  uart_puts(UART_ID, sval); // Send string 
 uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
 TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  0 \r\n");  // DVM_TRIG =0
...
sprintf(sval,"\nConnect DMM to input pins (+:J20-2 & -:J20-3)\n");  uart_puts(UART_ID, sval); // Send string 
sprintf(sval,"Set DMM to be able to read 10V\n");  uart_puts(UART_ID, sval); // Send string 
sprintf(sval,"Connect 12Vdc Power supply to PS1 (+:J17-1 & -:J17:3), press enter\n");  uart_puts(UART_ID, sval); // Send string
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  1 \r\n");  // Close K4 
TEST_SCPI_INPUT("DIG:OUT:PORT0 #HBD \r\n");// Close K3,K8,K14,K15,K16,K9
TEST_SCPI_INPUT("ROUT:CLOSE:OC OC1 \r\n"); // close K10 to set PS4
sprintf(sval,"Test 26.0 Verify if DMM value value is between 0V  and 0.1 Volt, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("ROUT:CLOSE:PWR SSR1 \r\n"); // Connect PS4 to 10 ohm and read to VM6
sprintf(sval,"Test 26.1 Verify if DMM value value is between 5V and 6V, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("ROUT:OPEN:PWR SSR1 \r\n"); // Connect PS4 to 10 ohm and read to VM6
...
sprintf(sval,"Test 26.2 Verify if DMM value value is between 0V  and 0.1 Volt, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("ROUT:CLOSE:PWR SSR1 \r\n"); // Connect PS4 to 10 ohm and read to VM6
sprintf(sval,"Test 26.3 Verify if DMM value value is between 4V and 5V, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("ROUT:OPEN:PWR SSR1 \r\n"); // Connect PS4 to 10 ohm and read to VM6
TEST_SCPI_INPUT("GPIO:OUT:DEV1:GP18  0 \r\n");  // Open K4 
...
sprintf(sval,"Set Vertical channel to 1V and timebase to 500us, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("DIG:OUT:PORT0 #H02 \r\n");// Close K7
TEST_SCPI_INPUT("COM:I2C:WRI 80,1\r\n"); //Set PWM ON
TEST_SCPI_INPUT("COM:I2C:WRI 81,1\r\n"); //Set PWM Freq to 1Khz
sprintf(sval,"Test 27.0 Verify on SCOPE CH1 if 3.3V@1KHz square wave is present, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char

sprintf(sval,"\nConnect Oscilloscope to CH2 (J19)\n");  uart_puts(UART_ID, sval); // Send string 
sprintf(sval,"Set Vertical channel to 1V and timebase to 5us, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("DIG:OUT:PORT0 #H06 \r\n");// Close K3,K7
TEST_SCPI_INPUT("COM:I2C:WRI 80,1\r\n"); //Set PWM ON
TEST_SCPI_INPUT("COM:I2C:WRI 81,100\r\n"); //Set PWM Freq to 100Khz
sprintf(sval,"Test 27.1 Verify on SCOPE CH2 if 3.3V@100KHz square wave is present, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
...
sprintf(sval,"Set Vertical channel to 1V and timebase to 50us\n");  uart_puts(UART_ID, sval); // Send string 
sprintf(sval,"Set Signal Generator to 10KHz sinus at 5Vpp, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
sprintf(sval,"Test 28.0 Verify on SCOPE CH1 if 5Vpp @10KHz sinus is present, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char

// Test of INST using oscilloscope CH1 
sprintf(sval,"\nConnect Signal Generator to SPARE input (J22), Connect Oscilloscope to CH1 (J18)\n");  uart_puts(UART_ID, sval); // Send string 
sprintf(sval,"Set Vertical channel to 1V and timebase to 50us\n");  uart_puts(UART_ID, sval); // Send string 
sprintf(sval,"Set Signal Generator to 10KHz triangle at 5Vpp, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("DIG:OUT:PORT0 #H40 \r\n");// Close K2
sprintf(sval,"Test 29.0 Verify on SCOPE CH1 if 5Vpp @10KHz triangle is present, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
TEST_SCPI_INPUT("DIG:OUT:PORT0 #H40 \r\n");// Open K2
...
sprintf(sval,"Connect USB Flash Drive to USB connector Type A on Selftest Board (J4), press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
sprintf(sval,"Test 30.0 Verify if computer as detected and could read the USB flash drive, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
...
sprintf(sval,"Connect a new network cable type RJ45 between selftest board (J3) and computer, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
sprintf(sval,"Test 31.0 Verify if computer network is ON and work properly, press enter\n");  uart_puts(UART_ID, sval); // Send string 
uart_read_blocking(UART_ID,in,1); sleep_ms(500);// wait for input char
...
test_cmd_out("Test 3.1 SCPI Error command ERR:COUNT?", "SYST:ERR:COUN?\r\n","0",&ctest, &buffer);
...
test_cmd_out("Test 3.3 SCPI Error command ERR:COUNT?", "SYST:ERR:COUN?\r\n","1",&ctest, &buffer);
...
fprintf(stdout, "\t Number of Tests ERROR:\t%d\r\n", ctest.error);
 ...
 // send result string to serial port
  sprintf(sval,"TEST COMMAND RESULTS: \n NbTotal: %d, NbGood: %d, NbBad: %d, NbError: %d\n",ctest.total,ctest.good,ctest.bad,ctest.error);  // build string to return
  uart_puts(UART_ID, sval); // Send result
  ...
  sprintf(sval,"TEST COMMAND COMPLETED \n");  // build string to return
  uart_puts(UART_ID, sval); // Send string
