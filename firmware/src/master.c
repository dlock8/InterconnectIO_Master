 ...
 * 
 * @details The Master Pico is the main controller for the Interconnec IO Box. The master accept SCPI command
 ...
 * 
 * This variable manages a circular queue that holds messages and tracks their sizes.
 * It uses indices to mark the beginning and end of the queue, and keeps track of the
 * current number of messages (load).
 */
struct {   // Global structure for circular queue
 ...
 * 
 * This function enqueues a message into a circular queue if there is space available.
 * It updates the queue's `end` index and `current_load`, managing wrap-around when
 * the queue reaches its maximum size.
 * 
 ...
 * 
 * This function sets the `begin`, `end`, and `current_load` indices of the queue 
 * to zero, indicating that the queue is empty. It also clears the message storage
 ...
 * 
 * This function dequeues a message from the circular queue if there is at least one 
 * message present. It retrieves the message and its size, resets the corresponding 
 * storage, and updates the `begin` index to point to the next message.
 * 
 * @param message Pointer to the structure where the dequeued message will be stored.
 * @param size Pointer to a variable where the size of the dequeued message will be stored.
 * 
 * @return true if the message was successfully removed from the queue; false if the queue is empty.
 */
...
void Hardware_Default_Setting() {
  ...
  if (valid == true) {  // if no error on boot Check, validate eeprom
    ...
    //cfg_eeprom_write_default();  // if new parameter added, write on eeprom
  ...
  valid =stringtonumber(ee.cfg.slave_force_run,sizeof(ee.cfg.slave_force_run), &vnum);   // read if toogle RUN_EN is enabled
