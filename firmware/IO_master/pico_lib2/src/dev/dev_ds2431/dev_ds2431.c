...
model_type _chip_model_list[] = {
    ...
    { 0x2D, "DS2431", 4, 2, false }, // 4 pages of 256 bit
...
/**
 * @brief Function to send a One-Wire reset pulse and detect if Onewrire respond (presence)
 *        
 * 
...
/**
 * @brief Function to write the device_id to the OneWire device
 *        before performing a read or write on the device
 * 
 * @param byte   pointer to the 8 Id bytes
 */
...
static bool read_eeprom_id(uint8_t id[]) {
    if (!onewire_reset()) {
        printf("Device %d not found on read_id\n");
        return false;
    }
...
static bool read_eeprom(uint8_t *device_id, uint8_t *data_buffer, int start_address, int length) {
    ...
    if (device_id[0] == 0) { // if device id not defined, expect single device 
...
static uint8_t write_eeprom_8bytes(const uint8_t *device_id, const char *data_buffer, int start_address, int length) {
 ...
 // volatile uint8_t crcw[2];
  ...
  // Write Scratchpad block
    ...
    for (int i = 0; i < length; i++) {
         if (data_read[i] != data_buffer[i]) { // if error found stop
...
static bool onewire_read_id(struct rom *romid, size_t nb_devices_expected){
    ...
    do { // loop is required due to intermittence in detection od Onewire devices
        ...
        if (valid == true) {
                ...
                while(onewire_search(id)){
                    ...
                    if (crc != 0) { romid->ecode[nb]= CRC_MISMATCH;} 
...
uint8_t onewire_write_info(const char* info,int start_address){
    ...
    for (i = 0; i < strlen(info); i += 2) {
        ...
        if (byte_index > 7 )   { break;}   // if device id is completed 
    }
...
uint8_t onewire_read_info(char** rinfo, int start_address,int length,size_t nbid){
    ...
    *rinfo = (char*)malloc(length*MAX_ONEWIRE); //set size of pointer
    ...
    if (valid == false) { 
        if (idr.nbid == 0) { rinfo[0] = '\0';return OW_NO_ONEWIRE; }
        else { rinfo[0] = '\0';return OW_NB_ONEWIRE;}
    }

    for (size_t nb =0; nb <idr.nbid; nb++)
    {
        if (idr.ecode[nb] == 0) {     // if no error detected
    ...
    for (size_t e =0; e <idr.nbid; e++) {
        if (idr.ecode[e] != 0) { return idr.ecode[e];} // if error exist, return error
    }
...
uint8_t onewire_check_devices(char** owdata, size_t nbid ){
    ...
    *owdata = (char*)malloc((16+16)*MAX_ONEWIRE); //set size of pointer to comntains the ID of device
    ...
    if (valid == false) {                 // if error received exit
        if (idr.nbid == 0) { owdata[0] = '\0';return OW_NO_ONEWIRE; }
        else { owdata[0] = '\0';return OW_NB_ONEWIRE;}
    }
    ...
    for (size_t nb =0; nb <idr.nbid; nb++) // lopp for each devices found on the search devices
    {
        if (idr.ecode[nb] == 0) {     // if no error detected
            ...
            for (int x =0; x < NB_TEST; x++) { teststr[x] = '\0';} // init teststr to NULL
           
            strncpy(teststr,&idr.idstr[nb][0],16); // Start string with device id
            strcat(teststr,", ");                  // add comma 
            strcat(teststr, randStr);               // add randowm number
